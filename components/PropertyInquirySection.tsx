...
export default function PropertyInquirySection() {
  ...
  const handleNextStep = () => {
    const newErrors: any = {
      ...
      exactLocation: !formData.exactLocation && (activeTab === "Buy" || activeTab === "Sell"),
    };
    if (activeTab === "Rent") {
      newErrors.rentalTerm = !formData.rentalTerm;
    }
    setErrors(newErrors);
  ...
  return (
    <section className="relative z-20 -mt-10 md:-mt-14 w-full max-w-[90vw] xl:max-w-[80vw] mx-auto">
            ...
            {(["Rent", "Buy", "Sell"] as const).map((tab) => (
              <button
                ...
                className={`flex-1 py-4 md:py-6 text-center text-xs font-bold uppercase tracking-[0.2em] transition-all border-r border-white/10 last:border-r-0 relative group ${
                  activeTab === tab 
                    ? "bg-white text-navy shadow-[0_-4px_20px_rgba(0,0,0,0.1)]" 
                  ...
                  <div className="flex flex-row flex-wrap items-end gap-4 md:gap-6 text-navy">
                    ...
                    <div className="flex flex-col gap-1.5 flex-1 min-w-[140px]">
                       <div className="space-y-1">
                          <label className="text-navy/40 text-[10px] font-bold uppercase tracking-widest ml-4">Country (UK)</label>
                          <div className="relative">
                            ...
                            <select 
                              ...
                              onChange={(e) => {
                                ...
                                if (e.target.value) setErrors((prev: any) => ({...prev, country: false}));
                              }}
                            ...
                            {errors.country && <span className="absolute -bottom-5 left-4 text-[9px] text-gold font-bold uppercase tracking-tighter">Please select a country</span>}
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5 flex-[1.5] min-w-[180px]">
                       <div className="space-y-1 relative">
                          <label className="text-navy/40 text-[10px] font-bold uppercase tracking-widest ml-4">
                            Exact Location {activeTab === "Rent" ? "(Optional)" : ""}
                          </label>
                          <input 
                            type="text"
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.value) setErrors((prev: any) => ({...prev, exactLocation: false}));
                            }}
                          ...
                          />
                          {errors.exactLocation && <span className="absolute -bottom-5 left-6 text-[9px] text-gold font-bold uppercase tracking-tighter w-max">Location is required for {activeTab}</span>}
                       </div>
                    </div>
                    ...
                    {activeTab === "Rent" && (
                      <div className="flex flex-col gap-1.5 flex-1 min-w-[140px]">
                        <div className="space-y-1">
                            <label className="text-navy/40 text-[10px] font-bold uppercase tracking-widest ml-4">Rental Term</label>
                            <div className="relative">
                              <select 
                                value={formData.rentalTerm}
                                onChange={(e) => {
                                  setFormData({...formData, rentalTerm: e.target.value});
                                  if (e.target.value) setErrors((prev: any) => ({...prev, rentalTerm: false}));
                                }}
                              ...
                              >
                                ...
                                {RENTAL_TERMS.map(t => <option key={t} value={t}>{t}</option>)}
                              </select>
                              {errors.rentalTerm && <span className="absolute -bottom-5 left-6 text-[9px] text-gold font-bold uppercase tracking-tighter">Specify your duration</span>}
                            </div>
                        </div>
                      </div>
                    )}
                    ...
                    {activeTab === "Sell" && (
                      <div className="flex flex-col gap-1.5 flex-[2] min-w-[180px]">
                        <div className="space-y-1">
                            <label className="text-navy/40 text-[10px] font-bold uppercase tracking-widest ml-4">Short Message</label>
                            <input 
                              type="text"
                              placeholder="Any initial details..."
                              value={formData.message}
                              onChange={(e) => setFormData({...formData, message: e.target.value})}
                              className="w-full bg-accent/30 border-b-2 border-transparent rounded-xl md:rounded-2xl px-6 py-3.5 text-navy placeholder:text-navy/20 focus:outline-none focus:bg-accent/50 font-medium text-sm transition-all"
                            />
                        </div>
                      </div>
                    )}
      ...
      <PropertyInquiryModal 
        ...
        type={activeTab.toLowerCase() as any}
